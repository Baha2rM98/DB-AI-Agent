...
def initialize_async_agent(
    checkpointer: Optional[Any] = None,
    model_name: str = "gemini-1.5-pro",
):
    """Initialize an async LangGraph agent for non-blocking request handling."""
    ...
    async def understand_query(state: AgentState) -> AgentState:
        """Parse and understand the user's natural language query asynchronously."""
        prompt = ChatPromptTemplate.from_messages([
            ("system", system_message),
            ("user", "{query}")
        ])
    ...
    async def plan_execution(state: AgentState) -> AgentState:
        """Develop a plan for database operations asynchronously."""
        if "understood_intent" not in state.context:
            state.error = "Query understanding failed. Cannot create execution plan."
            return state
    ...
    async def execute_plan(state: AgentState) -> AgentState:
        """Execute the planning step asynchronously."""
        if "execution_plan" not in state.context:
            state.error = "Execution plan missing. Cannot proceed with execution."
            return state
    ...
    async def handle_error(state: AgentState) -> AgentState:
        """Handle any errors that occurred during async processing."""
        error_prompt = ChatPromptTemplate.from_messages([
            ("system", """You are a database troubleshooter.
            Your job is to diagnose database query issues and provide helpful explanations.
...
async def query_database_async(
    ...
    try:
        logger.info("Initializing async LangGraph agent for thread %s", thread_id or "default")
        agent = initialize_async_agent(checkpointer=checkpointer, model_name=model_name)
        ...
        logger.debug("Async agent result type: %s", type(result))
        ...
        logger.warning("Unexpected result type from async agent: %s", type(result))
        return {
            "response": "The agent could not process your query properly.",
            "context": {},
            "execution_details": {}
        ...
        import traceback
        logger.error("Error in query_database_async: %s", e)
        logger.debug(traceback.format_exc())
        return {
            "response": f"Error processing query: {str(e)}",
            "context": {"error": str(e)},
